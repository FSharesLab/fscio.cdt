...
#include <fsciolib/permission.h>
#include <fsciolib/print.h>
#include <fsciolib/privileged.h>
#include <fsciolib/system.h>
#include <fsciolib/transaction.h>
#include <fsciolib/types.h>
...
extern "C" {
   ...
   float _fscio_f32_max( float af, float bf ) {
      float32_t a = to_softfloat32(af);
      float32_t b = to_softfloat32(bf);
      if (f32_is_nan(a)) {
         return af;
   ...
   float _fscio_f32_copysign( float af, float bf ) {
      float32_t a = to_softfloat32(af);
      float32_t b = to_softfloat32(bf);
   ...
   // float unops
   float _fscio_f32_abs( float af ) {
      float32_t a = to_softfloat32(af);
      a.v &= ~(1 << 31);
      return from_softfloat32(a);
   }
   float _fscio_f32_neg( float af ) {
      float32_t a = to_softfloat32(af);
   ...
   float _fscio_f32_floor( float af ) {
      float32_t a = to_softfloat32(af);
      int e = (int)(a.v >> 23 & 0xFF) - 0X7F;
      uint32_t m;
      if (e >= 23)
   ...
   float _fscio_f32_trunc( float af ) {
      float32_t a = to_softfloat32(af);
   ...
   float _fscio_f32_nearest( float af ) {
      float32_t a = to_softfloat32(af);
      int e = a.v>>23 & 0xff;
      int s = a.v>>31;
      float32_t y;
   ...
   // float relops
   bool _fscio_f32_eq( float a, float b ) {  return f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
   bool _fscio_f32_ne( float a, float b ) { return !f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
   bool _fscio_f32_lt( float a, float b ) { return f32_lt( to_softfloat32(a), to_softfloat32(b) ); }
   bool _fscio_f32_le( float a, float b ) { return f32_le( to_softfloat32(a), to_softfloat32(b) ); }
   bool _fscio_f32_gt( float af, float bf ) {
      float32_t a = to_softfloat32(af);
      float32_t b = to_softfloat32(bf);
      if (f32_is_nan(a))
         return false;
   ...
   bool _fscio_f32_ge( float af, float bf ) {
      float32_t a = to_softfloat32(af);
      float32_t b = to_softfloat32(bf);
      if (f32_is_nan(a))
         return false;
   ...
   double _fscio_f64_max( double af, double bf ) {
      float64_t a = to_softfloat64(af);
      float64_t b = to_softfloat64(bf);
      if (f64_is_nan(a))
         return af;
   ...
   double _fscio_f64_copysign( double af, double bf ) {
      float64_t a = to_softfloat64(af);
      float64_t b = to_softfloat64(bf);
   ...
   // double unops
   double _fscio_f64_abs( double af ) {
      float64_t a = to_softfloat64(af);
      a.v &= ~(uint64_t(1) << 63);
      return from_softfloat64(a);
   }
   double _fscio_f64_neg( double af ) {
      float64_t a = to_softfloat64(af);
   ...
   double _fscio_f64_sqrt( double a ) {
      float64_t ret = f64_sqrt( to_softfloat64(a) );
      return from_softfloat64(ret);
   }
   // ceil, floor, trunc and nearest are lifted from libc
   double _fscio_f64_ceil( double af ) {
      float64_t a = to_softfloat64( af );
      float64_t ret;
      int e = a.v >> 52 & 0x7ff;
      float64_t y;
   ...
   double _fscio_f64_floor( double af ) {
      float64_t a = to_softfloat64( af );
      float64_t ret;
      int e = a.v >> 52 & 0x7FF;
      float64_t y;
   ...
   double _fscio_f64_trunc( double af ) {
      float64_t a = to_softfloat64( af );
   ...
   double _fscio_f64_nearest( double af ) {
      float64_t a = to_softfloat64( af );
      int e = (a.v >> 52 & 0x7FF);
      int s = a.v >> 63;
      float64_t y;
   ...
   // double relops
   bool _fscio_f64_eq( double a, double b ) { return f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
   bool _fscio_f64_ne( double a, double b ) { return !f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
   bool _fscio_f64_lt( double a, double b ) { return f64_lt( to_softfloat64(a), to_softfloat64(b) ); }
   bool _fscio_f64_le( double a, double b ) { return f64_le( to_softfloat64(a), to_softfloat64(b) ); }
   bool _fscio_f64_gt( double af, double bf ) {
      float64_t a = to_softfloat64(af);
      float64_t b = to_softfloat64(bf);
      if (f64_is_nan(a))
         return false;
   ...
   bool _fscio_f64_ge( double af, double bf ) {
      float64_t a = to_softfloat64(af);
      float64_t b = to_softfloat64(bf);
      if (f64_is_nan(a))
         return false;
   ...
   double _fscio_f32_promote( float a ) {
      return from_softfloat64(f32_to_f64( to_softfloat32(a)) );
   }
   float _fscio_f64_demote( double a ) {
      return from_softfloat32(f64_to_f32( to_softfloat64(a)) );
   }
   int32_t _fscio_f32_trunc_i32s( float af ) {
      float32_t a = to_softfloat32(af);
      if (_fscio_f32_ge(af, 2147483648.0f) || _fscio_f32_lt(af, -2147483648.0f))
         fscio_assert(false,  "Error, f32.convert_s/i32 overflow" );

      if (f32_is_nan(a))
         fscio_assert(false,  "Error, f32.convert_s/i32 unrepresentable");
      return f32_to_i32( to_softfloat32(_fscio_f32_trunc( af )), 0, false );
   }
   int32_t _fscio_f64_trunc_i32s( double af ) {
      float64_t a = to_softfloat64(af);
      if (_fscio_f64_ge(af, 2147483648.0) || _fscio_f64_lt(af, -2147483648.0))
         fscio_assert(false,  "Error, f64.convert_s/i32 overflow");
      if (f64_is_nan(a))
         fscio_assert(false,  "Error, f64.convert_s/i32 unrepresentable");
      return f64_to_i32( to_softfloat64(_fscio_f64_trunc( af )), 0, false );
   }
   uint32_t _fscio_f32_trunc_i32u( float af ) {
      float32_t a = to_softfloat32(af);
      if (_fscio_f32_ge(af, 4294967296.0f) || _fscio_f32_le(af, -1.0f))
         fscio_assert(false,  "Error, f32.convert_u/i32 overflow");
      if (f32_is_nan(a))
         fscio_assert(false,  "Error, f32.convert_u/i32 unrepresentable");
      return f32_to_ui32( to_softfloat32(_fscio_f32_trunc( af )), 0, false );
   }
   uint32_t _fscio_f64_trunc_i32u( double af ) {
      float64_t a = to_softfloat64(af);
      if (_fscio_f64_ge(af, 4294967296.0) || _fscio_f64_le(af, -1.0))
         fscio_assert(false,  "Error, f64.convert_u/i32 overflow");
      if (f64_is_nan(a))
         fscio_assert(false,  "Error, f64.convert_u/i32 unrepresentable");
      return f64_to_ui32( to_softfloat64(_fscio_f64_trunc( af )), 0, false );
   }
   int64_t _fscio_f32_trunc_i64s( float af ) {
      float32_t a = to_softfloat32(af);
      if (_fscio_f32_ge(af, 9223372036854775808.0f) || _fscio_f32_lt(af, -9223372036854775808.0f))
         fscio_assert(false,  "Error, f32.convert_s/i64 overflow");
      if (f32_is_nan(a))
         fscio_assert(false,  "Error, f32.convert_s/i64 unrepresentable");
      return f32_to_i64( to_softfloat32(_fscio_f32_trunc( af )), 0, false );
   }
   int64_t _fscio_f64_trunc_i64s( double af ) {
      float64_t a = to_softfloat64(af);
      if (_fscio_f64_ge(af, 9223372036854775808.0) || _fscio_f64_lt(af, -9223372036854775808.0))
         fscio_assert(false,  "Error, f64.convert_s/i64 overflow");
      if (f64_is_nan(a))
         fscio_assert(false,  "Error, f64.convert_s/i64 unrepresentable");

      return f64_to_i64( to_softfloat64(_fscio_f64_trunc( af )), 0, false );
   }
   uint64_t _fscio_f32_trunc_i64u( float af ) {
      float32_t a = to_softfloat32(af);
      if (_fscio_f32_ge(af, 18446744073709551616.0f) || _fscio_f32_le(af, -1.0f))
         fscio_assert(false,  "Error, f32.convert_u/i64 overflow");
      if (f32_is_nan(a))
         fscio_assert(false,  "Error, f32.convert_u/i64 unrepresentable");
      return f32_to_ui64( to_softfloat32(_fscio_f32_trunc( af )), 0, false );
   }
   uint64_t _fscio_f64_trunc_i64u( double af ) {
      float64_t a = to_softfloat64(af);
      if (_fscio_f64_ge(af, 18446744073709551616.0) || _fscio_f64_le(af, -1.0))
         fscio_assert(false,  "Error, f64.convert_u/i64 overflow");
      if (f64_is_nan(a))
         fscio_assert(false,  "Error, f64.convert_u/i64 unrepresentable");
      return f64_to_ui64( to_softfloat64(_fscio_f64_trunc( af )), 0, false );
   }
   float _fscio_i32_to_f32( int32_t a )  {
      return from_softfloat32(i32_to_f32( a ));
   }
   float _fscio_i64_to_f32( int64_t a ) {
      return from_softfloat32(i64_to_f32( a ));
   }
   float _fscio_ui32_to_f32( uint32_t a ) {
      return from_softfloat32(ui32_to_f32( a ));
   }
   float _fscio_ui64_to_f32( uint64_t a ) {
      return from_softfloat32(ui64_to_f32( a ));
   }
   double _fscio_i32_to_f64( int32_t a ) {
      return from_softfloat64(i32_to_f64( a ));
   }
   double _fscio_i64_to_f64( int64_t a ) {
      return from_softfloat64(i64_to_f64( a ));
   }
   double _fscio_ui32_to_f64( uint32_t a ) {
      return from_softfloat64(ui32_to_f64( a ));
   }
   double _fscio_ui64_to_f64( uint64_t a ) {
   ...
   void fscio_assert(uint32_t test, const char* msg) {
      if (test == 0) {
         _prints(msg, fscio::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, fscio::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }

   void fscio_assert_message(uint32_t test, const char* msg, uint32_t len) {
      if (test == 0) {
         _prints_l(msg, len, fscio::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, fscio::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }
   ...
   void fscio_assert_code(uint32_t test, uint64_t code) {
      if (test == 0) {
         ...
         _prints(buff, fscio::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, fscio::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }
...
#pragma clang diagnostic pop
   ...
   size_t __builtin_wasm_grow_memory(size_t size) {
      if ((___heap_ptr + (size*64*1024)) > (___heap_ptr + 100*1024*1024))
         fscio_assert(false, "__builtin_wasm_grow_memory");
