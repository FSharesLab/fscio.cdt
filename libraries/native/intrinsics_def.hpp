...
namespace fscio { namespace native {
   ...
   auto create_function(std::index_sequence<Is...>) {
      return std::function<R(typename std::tuple_element<Is, Args>::type ...)>{ 
         [](typename std::tuple_element<Is, Args>::type ...) { 
            fscio_assert(false, "unsupported intrinsic"); return (R)0;
         }
      };
   }   
...
#define GENERATE_TYPE_MAPPING(name) \
   struct __ ## name ## _types { \
      using deduced_full_ts = decltype(fscio::native::get_args_full(::name)); \
      using deduced_ts      = decltype(fscio::native::get_args(::name)); \
...
#define GET_TYPE(name) \
   decltype(create_function<fscio::native::intrinsics::__ ## name ## _types::res_t, \
         fscio::native::intrinsics::__ ## name ## _types::deduced_full_ts>(fscio::native::intrinsics::__ ## name ## _types::is)),

#define REGISTER_INTRINSIC(name) \
   create_function<fscio::native::intrinsics::__ ## name ## _types::res_t, \
         fscio::native::intrinsics::__ ## name ## _types::deduced_full_ts>(fscio::native::intrinsics::__ ## name ## _types::is),
