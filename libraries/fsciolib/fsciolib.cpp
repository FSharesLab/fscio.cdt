...
namespace fscio {
   ...
   void set_blockchain_parameters(const fscio::blockchain_parameters& params) {
      char buf[sizeof(fscio::blockchain_parameters)];
      fscio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(fscio::blockchain_parameters& params) {
      char buf[sizeof(fscio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      fscio::check( size <= sizeof(buf), "buffer is too small" );
      fscio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            fscio::check(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return fscio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = fscio::memory_heap.malloc(count*size);
...
{
   return fscio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return fscio::memory_heap.free(ptr);
}
