...
namespace fscio {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         fscio::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         fscio::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         fscio::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         fscio::check( -max_amount <= amount, "subtraction underflow" );
         fscio::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         fscio::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         fscio::check( -max_amount <= amount, "addition underflow" );
         fscio::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         fscio::check( tmp <= max_amount, "multiplication overflow" );
         fscio::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         fscio::check( a != 0, "divide by zero" );
         fscio::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         fscio::check( b.amount != 0, "divide by zero" );
         fscio::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
       ...
       */
      void print()const {
         fscio::print(to_string());
      }

      FSCLIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         fscio::check( a.contract == b.contract, "type mismatch" );
      ...
      FSCLIB_SERIALIZE( extended_asset, (quantity)(contract) )
   };
