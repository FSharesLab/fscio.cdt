...
namespace fscio {
   ...
   */
   ...
   public:
      ...
      {
         if( str.size() > 7 ) {
            fscio::check( false, "string is too long to be a valid symbol_code" );
         }
         for( auto itr = str.rbegin(); itr != str.rend(); ++itr ) {
            if( *itr < 'A' || *itr > 'Z') {
               fscio::check( false, "only uppercase letters allowed in symbol_code string" );
            }
            value <<= 8;
            value |= *itr;
         }
   ...
   private:
      ...
      FSCLIB_SERIALIZE( extended_symbol, (symbol)(contract) )
   };
