   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        fscio::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        fscio::check( _end - _pos >= (int32_t)s, "write" );
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const fscio::binary_extension<T>& be) {
  ds << be.value_or();
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, fscio::binary_extension<T>& be) {
  if( ds.remaining() ) {
     T val;
     ds >> val;
     be.emplace(val);
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      fscio::check(false, "invalid variant index");
   }
}
...
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const fscio::symbol_code sym_code) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, fscio::symbol_code& sym_code) {
  uint64_t raw = 0;
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const fscio::symbol sym) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, fscio::symbol& sym) {
  uint64_t raw = 0;
  ds.read((char*)&raw, sizeof(raw));
  sym = symbol(raw);
  return ds;
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::fscio::ignore_wrapper<T>& val) {
  ds << val.value;
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::fscio::ignore<T>& val) {
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, ::fscio::ignore<T>) {
  return ds;
}
...
/**
 *  Serialize an fscio::public_key into a stream
 *
 *  @brief Serialize an fscio::public_key
 *  @param ds - The stream to write
 *  @param pubkey - The value to serialize
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const fscio::public_key& pubkey) {
   ds << pubkey.type;
...
/**
 *  Deserialize an fscio::public_key from a stream
 *
 *  @brief Deserialize an fscio::public_key
 *  @param ds - The stream to read
 *  @param pubkey - The destination for deserialized value
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, fscio::public_key& pubkey) {
   ds >> pubkey.type;
...
/**
 *  Serialize an fscio::signature into a stream
 *
 *  @brief Serialize an fscio::signature
 *  @param ds - The stream to write
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const fscio::signature& sig) {
   ds << sig.type;
...
/**
 *  Deserialize an fscio::signature from a stream
 *
 *  @brief Deserialize an fscio::signature
 *  @param ds - The stream to read
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, fscio::signature& sig) {
   ds >> sig.type;
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   fscio::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
