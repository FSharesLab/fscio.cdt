...
namespace fscio {
   ...
   */
   ...
   public:
      ...
      FSCLIB_SERIALIZE( transaction_header, (expiration)(ref_block_num)(ref_block_prefix)(max_net_usage_words)(max_cpu_usage_ms)(delay_sec) )
   };
   ...
   public:
      ...
      FSCLIB_SERIALIZE_DERIVED( transaction, transaction_header, (context_free_actions)(actions)(transaction_extensions) )
   };
   ...
   struct onerror {
      ...
      */
      ...
      FSCLIB_SERIALIZE( onerror, (sender_id)(sent_trx) )
   };
   ...
   inline action get_action( uint32_t type, uint32_t index ) {
      ...
      fscio::check( s > 0, "get_action size failed" );
      size_t size = static_cast<size_t>(s);
      char* buffer = (char*)( max_stack_buffer_size < size ? malloc(size) : alloca(size) );
      auto size2 = ::get_action( type, index, buffer, size );
      fscio::check( size == static_cast<size_t>(size2), "get_action failed" );
      return fscio::unpack<fscio::action>( buffer, size );
   }
