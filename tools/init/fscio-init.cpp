...
#include "fscio/gen.hpp"
#include "fscio/whereami/whereami.hpp"
#include "fscio/abi.hpp"
...
struct project {
   ...
   const std::string hpp = "#include <fsciolib/fscio.hpp>\n"
                           "using namespace fscio;\n\n"
   ...
   const std::string cmake = "project(@)\n\n"
                             "set(FSCIO_WASM_OLD_BEHAVIOR \"Off\")\n"
   ...
   const std::string cmake_extern = "include(ExternalProject)\n"
                                    ...
                                    "if(FSCIO_CDT_ROOT STREQUAL \"\" OR NOT FSCIO_CDT_ROOT)\n"
                                    "   find_package(fscio.cdt)\n"
                                    "endif()\n\n"
                                    "ExternalProject_Add(\n"
                                    "   @_project\n"
                                    ...
                                    "   CMAKE_ARGS -DCMAKE_TOOLCHAIN_FILE=${FSCIO_CDT_ROOT}/lib/cmake/fscio.cdt/FscioWasmToolchain.cmake\n"
                                    "   UPDATE_COMMAND \"\"\n"
                                    "   PATCH_COMMAND \"\"\n"
                                    "   TEST_COMMAND \"\"\n"
                                    "   INSTALL_COMMAND \"\"\n"
   ...
   const std::string readme_cmake = "--- @ Project ---\n\n"
                                    ...
                                    "   - You can then do a 'set contract' action with 'clfsc' and point in to the './build/@' directory\n\n"
                                    " - Additions to CMake should be done to the CMakeLists.txt in the './src' directory and not in the top level CMakeLists.txt";

   const std::string readme_bare = " --- @ Project ---\n\n"
                                   " - How to Build -\n"
                                   "   - run the command 'fscio-cpp -abigen -o @.wasm @.cpp'\n";
...
int main(int argc, const char **argv) {
   ...
   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
        os << "fscio-init version " << "@VERSION_FULL@" << "\n";
  });
   cl::OptionCategory cat("fscio-init", "generates an fscio smart contract project");
   ...
   cl::ParseCommandLineOptions(argc, argv, std::string("fscio-proj"));
   try {
