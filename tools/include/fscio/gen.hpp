...
#include <utility>
#include <regex>
#include <fscio/utils.hpp>

namespace fscio { namespace cdt {
...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isFscioIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isFscioIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_fscio_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasFscioRicardian();
   }
   static inline bool has_fscio_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasFscioRicardian();
   }

   static inline std::string get_fscio_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getFscioRicardianAttr()->getName();
   }
   static inline std::string get_fscio_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getFscioRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getFscioActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline bool is_fscio_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isFscioContract())
         name = decl->getFscioContractAttr()->getName();
      else if (decl->getParent()->isFscioContract())
         name = decl->getParent()->getFscioContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_fscio_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isFscioContract()) {
         auto nm = decl->getFscioContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isFscioContract()) {
         auto nm = pd->getFscioContractAttr()->getName().str();
