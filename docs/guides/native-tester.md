...
As of v1.5.0 native compilation can be performed and a new set of libraries to facilitate native testing and native "scratch pad" compilation.  `fscio-cc\cpp` and `fscio-ld` now support building "smart contracts" and unit tests natively for quick tests to help facilitate faster development \(note the default implementations of fscio `intrinsics` are currently asserts that state they are unavailable, these are user definable.\)
...
CONTRACT hello : public fscio::contract {
  public:
...
FSCIO_TEST_BEGIN(hello_test)
   ...
   intrinsics::set_intrinsic<intrinsics::read_action_data>(
         [](void* m, uint32_t len) {
            check(len <= sizeof(fscio::name), "failed from read_action_data");
            *((fscio::name*)m) = "hello"_n;
            return len; 
         });

   intrinsics::set_intrinsic<intrinsics::action_data_size>(
         []() {
            return (uint32_t)sizeof(fscio::name);
         });
...
- FSCIO_TEST_BEGIN(X) : This macro defines the beginning of a unit test and assigns `X` as the symbolic name of that test.
- FSCIO_TEST_END : This macro defines the end of a unit test.
- FSCIO_TEST(X) : This is used to run a particular named unit test `X` in the main function.
