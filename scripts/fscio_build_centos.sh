	...
	if [ "${OS_VER}" -lt 7 ]; then
		printf "\\n\\tYou must be running Centos 7 or higher to install FSCIO.\\n"
		printf "\\tExiting now.\\n\\n"
		exit 1;
	fi

	if [ "${DISK_AVAIL%.*}" -lt "${DISK_MIN}" ]; then
		printf "\\n\\tYou must have at least %sGB of available storage to install FSCIO.\\n" "${DISK_MIN}"
		printf "\\tExiting now.\\n\\n"
		exit 1;
	fi
	...
	then
		printf "\\n\\tYum must be installed to compile FSC.IO.\\n"
		printf "\\tExiting now.\\n\\n"
		exit 1;
	fi
	...
	if [ -z "${SCL}" ]; then
		printf "\\n\\tThe Centos Software Collections Repository, devtoolset-7 and Python3 are required to install FSCIO.\\n"
