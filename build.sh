...
if [[ "${unamestr}" == 'Darwin' ]]; then
   ...
   CXX_COMPILER=g++
   export ARCH="Darwin"
   bash ./scripts/fscio_build_darwin.sh
else
   ...
   case "$OS_NAME" in
      "Amazon Linux AMI")
         ...
         bash ./scripts/fscio_build_amazon.sh
         ;;
      "CentOS Linux")
         export ARCH="Centos"
         export CMAKE=${HOME}/opt/cmake/bin/cmake
         bash ./scripts/fscio_build_centos.sh
         ;;
      "elementary OS")
         export ARCH="elementary OS"
         bash ./scripts/fscio_build_ubuntu.sh
         ;;
      "Fedora")
         export ARCH="Fedora"
         bash ./scripts/fscio_build_fedora.sh
         ;;
      "Linux Mint")
...
"$CMAKE" -DCMAKE_INSTALL_PREFIX=/usr/local/fscio.cdt ../
if [ $? -ne 0 ]; then
   exit -1;
fi
make -j${CORES}
