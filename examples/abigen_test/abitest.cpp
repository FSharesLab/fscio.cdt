...
namespace test {
   ...
   struct [[ using fscio: action("testc"), contract("abitest") ]] test_c : testb {
      uint64_t num;
   };
}

CONTRACT abitest : public fscio::contract {
  public:
      using contract::contract;
      
      // mark this method as an action and specify the name explicity
      [[ fscio::action("testacta") ]]
      ...
      struct [[fscio::table("testtabb")]] test_table2 {
         uint64_t owner;
         uint64_t sec;
         uint64_t third;
         uint64_t primary_key() const { return owner; }
...
typedef fscio::multi_index< "testtab"_n,  abitest::testtable > testtable_t;
typedef fscio::multi_index< "testtaba"_n, abitest::testtable > testtable_a_t;
typedef fscio::multi_index< "testtab2"_n, abitest::test_table2 > testtable2_t;
