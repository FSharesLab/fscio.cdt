...
void f(int val, MoveOnly moParam, MoveAndCopy mcParam) {
  ...
  // CHECK: call void @_ZN8MoveOnlyC1FSC_(%struct.MoveOnly* %[[MoCopy]], %struct.MoveOnly* dereferenceable(4) %[[MoParam]])
  // CHECK-NEXT: call void @_ZN11MoveAndCopyC1FSC_(%struct.MoveAndCopy* %[[McCopy]], %struct.MoveAndCopy* dereferenceable(4) %[[McParam]]) #
...
void dependent_params(T x, U, U y) {
  ...
  // CHECK: call i8* @llvm.coro.begin
  // CHECK-NEXT: call void @_ZN1AC1FSC_(%struct.A* %[[x_copy]], %struct.A* dereferenceable(512) %x)
  // CHECK-NEXT: call void @_ZN1BC1FSC_(%struct.B* %[[unnamed_copy]], %struct.B* dereferenceable(512) %0)
  // CHECK-NEXT: call void @_ZN1BC1FSC_(%struct.B* %[[y_copy]], %struct.B* dereferenceable(512) %y)
