...
// RUN: %clang -target x86_64-apple-darwin -arch arm64 -miphonfsc-version-min=7.0 %s -### 2>&1 | FileCheck %s --check-prefix=CHECK-LIBCXX
...
// RUN: %clang -target x86_64-apple-darwin -arch armv7s -miphonfsc-version-min=6.1 %s -### 2>&1 | FileCheck %s --check-prefix=CHECK-LIBSTDCXX
// RUN: %clang -target x86_64-apple-darwin -arch armv7s -miphonfsc-version-min=7.0 %s -### 2>&1 | FileCheck %s --check-prefix=CHECK-LIBCXX
