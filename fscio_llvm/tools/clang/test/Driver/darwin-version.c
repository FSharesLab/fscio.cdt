...
// RUN: %clang -target armv7-apple-darwin -miphonfsc-version-min=11.0 -c -### %s 2> %t.err
// RUN: FileCheck --input-file=%t.err --check-prefix=CHECK-VERSION-IOS5 %s
// CHECK-VERSION-IOS5: invalid iOS deployment version '-miphonfsc-version-min=11.0'
...
// RUN: %clang -target arm64-apple-ios11.0 -miphonfsc-version-min=11.0 -c -### %s 2>&1 | \
...
// RUN: %clang -target armv7-apple-ios9.0 -miphonfsc-version-min=11.0 -c -Wno-invalid-ios-deployment-target -### %s 2>&1 | \
...
// RUN: env MACOSX_DEPLOYMENT_TARGET=10.5 IPHONFSC_DEPLOYMENT_TARGET=2.0 \
...
// RUN: env IPHONFSC_DEPLOYMENT_TARGET=2.3.1 \
// RUN:   %clang -target armv6-apple-darwin9 -c %s -### 2>&1 | \
...
// RUN: %clang -target x86_64-apple-macos -miphonfsc-version-min=9.1 -c %s -### 2>&1 | \
// RUN:   FileCheck --check-prefix=CHECK-VERSION-TNO-OSV2 %s
// CHECK-VERSION-TNO-OSV2: overriding '-miphonfsc-version-min=9.1' option with '--target=x86_64-apple-macos'
...
// RUN: %clang -target arm64-apple-ios10.1.0 -miphonfsc-version-min=10.1.0.1 -c %s -### 2>&1 | \
// RUN:   FileCheck --check-prefix=CHECK-VERSION-TNO-OSV4 %s
// CHECK-VERSION-TNO-OSV4: overriding '-miphonfsc-version-min=10.1.0.1' option with '--target=arm64-apple-ios10.1.0'
...
// RUN: env IPHONFSC_DEPLOYMENT_TARGET=10.1 \
